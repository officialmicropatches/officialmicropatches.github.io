...
const CATEGORIES = [
  ...
  { key: 'grade_fairness', label: 'Fairness', desc: 'Are policies and discipline applied consistently and fairly?' },
  { key: 'grade_accountability', label: 'Accountability', desc: 'Does leadership hold itself to the same standards?' },
  { key: 'grade_backing_officers', label: 'Backing Officers', desc: 'Does command back officers who act in good faith?' },
  { key: 'grade_toxic_behavior', label: 'Toxic Behavior', desc: 'Is the command climate free of bullying, retaliation, and manipulation?' },
  { key: 'grade_promotion_fairness', label: 'Promotion Fairness', desc: 'Are promotions merit-based and free of favoritism?' },
  { key: 'grade_work_life', label: 'Work/Life Balance', desc: 'Does leadership support reasonable schedules and time off?' },
  { key: 'grade_morale_impact', label: 'Morale Impact', desc: 'Overall, does leadership positively or negatively affect morale?' },
];
...
export default function RateAgencyModal({ agencyId, agencyName, onClose, onSuccess }) {
  ...
  if (success) {
    return (
      <ModalShell onClose={onClose} title="Review Submitted">
        <p className="text-sm text-text-secondary mb-4">Your anonymous review has been submitted. Thank you for helping officers make informed decisions.</p>
  ...
  return (
    <ModalShell onClose={onClose} title={`Rate ${agencyName}`}>
      {step === 0 && (
        <div>
          <div className="p-4 bg-bg border border-border rounded-xl mb-6 text-sm text-text-secondary leading-relaxed">
            Ratings are anonymous. Please rate only what leadership controls — not equipment, pay, or facilities.
          </div>
          <button
            onClick={() => setStep(1)}
            className="w-full bg-navy text-white text-sm py-3 rounded-full hover:bg-navy-hover transition-colors"
          >
            Start Rating
          </button>
        </div>
      ...
      {step === 2 && (
        <div>
          ...
          <textarea
            ...
            placeholder="What would current officers or laterals want to know about this agency's leadership?"
            rows={5}
